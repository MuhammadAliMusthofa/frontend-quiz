...
function LobbyPage() {
  ...
  const fetchGameData = async () => {
    try {
      const response = await fetch(`http://103.235.73.11:4001/api/quiz_title/${gameId}`);
      if (response.ok) {
  ...
  const fetchPlayers = async () => {
    try {
      const response = await fetch(`http://103.235.73.11:4001/api/players/${gameId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const startGame = async () => {
    try {
      const response = await fetch(`http://103.235.73.11:4001/api/start/${gameCode}`, {
        method: "PUT",
      });
      if (response.ok) {
